...
class YandexRAGService:
    ...
    async def query_rag(self, department_id: int, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Формируем информацию об изображениях для промпта
            images_info = ""
            if total_images > 0:
                images_info = "\n\n### ИНФОРМАЦИЯ ОБ ИЗОБРАЖЕНИЯХ:\n"
                for i, source in enumerate(sources):
                    if source.get("images"):
                        images_info += f"\nИсточник {i+1} ({source['file_name']}):\n"
                        for j, img in enumerate(source["images"]):
                            img_context = img.get("context", "Без контекста")
                            relevance = img.get("relevance_score", 0.0)
                            images_info += f"- Изображение {j+1}: {img_context[:100]}... (релевантность: {relevance:.2f})\n"
